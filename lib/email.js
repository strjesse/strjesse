...
function buildConfirmation(opts) {
  ...
  var FORM_URL = "https://forms.gle/TJo3giTY3LUhFsYD9";
  var steps = [
    ["Add the invite", "The Google Calendar invite is already in your inbox — accept it so you get a reminder.", "", ""],
    ["Complete the pre-session questionnaire", "A few quick questions so Jesse can tailor the session to your situation.", "Open the questionnaire", FORM_URL],
    ["Bring any additional questions", "Note down anything specific you'd like to cover — the more specific, the better.", "", ""]
  ];
  ...
  var text =
    ...
    "2. Complete the pre-session questionnaire: " + FORM_URL + "\n" +
    "3. Bring any additional questions you'd like to cover.\n\n" +
      ...
      '<tr><td style="padding:0 0 14px;" valign="top">' +
        '<table role="presentation" cellpadding="0" cellspacing="0"><tr>' +
          ...
          '<td style="padding-left:12px;">' +
            ...
            (s[3] ? '<p style="margin:6px 0 0;font-size:14px;"><a href="' + esc(s[3]) + '" style="color:#B5400F;font-weight:bold;text-decoration:none;">' + esc(s[2]) + ' &rarr;</a></p>' : '') +
          '</td>' +
        '</tr></table>' +
      '</td></tr>';
  }).join("");
...
// Notify the business that someone joined the waitlist. Reply-To is set to
// the applicant so a reply goes straight to them. (applicant email is
// validated upstream, so it is safe to place in headers.)
async function sendWaitlist(applicantEmail) {
  var from = process.env.GOOGLE_IMPERSONATE_SUBJECT;
  var to = process.env.WAITLIST_TO || from;
  var lines = [
    "From: STRJesse <" + from + ">",
    "To: " + to,
    "Reply-To: " + applicantEmail,
    "Subject: New waitlist signup — " + applicantEmail,
    "MIME-Version: 1.0",
    "Content-Type: text/plain; charset=UTF-8",
    "",
    "Someone joined the STRJesse waitlist.\n\n" +
...
module.exports = { emailConfigured, buildConfirmation, sendConfirmation, sendWaitlist };
