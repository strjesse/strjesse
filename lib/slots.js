...
// Bookable start times each day, as human labels (90-min sessions
// spanning 10:30am - 6pm). Edit freely.
const SLOTS = ["10:30 AM", "12:00 PM", "1:30 PM", "3:00 PM", "4:30 PM"];
...
// The timezone the above labels are expressed in. AEST = UTC+10, no DST.
const TZ = process.env.BUSINESS_TIMEZONE || "Australia/Brisbane";

// Days of week that are NOT bookable (0 = Sunday ... 6 = Saturday).
// Open Tue/Wed/Thu/Sat, so Sun(0), Mon(1) and Fri(5) are closed.
const CLOSED_DOW = (process.env.CLOSED_DOW || "0,1,5")
  .split(",")
...
// No same-day bookings: the earliest bookable calendar date is tomorrow
...
function isBookableDate(dateStr) {
  return dateStr >= minBookableDate();
}

module.exports = { SLOTS, SESSION_MINUTES, TZ, CLOSED_DOW, parseLabel, slotToInterval, minBookableDate, isBookableDate };
